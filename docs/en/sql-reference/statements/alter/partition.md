...
-   [DETACH PARTITION](#detach-partition) — Moves a partition to the `detached` directory and forget it.
-   [DROP PARTITION](#drop-partition) — Deletes a partition.
-   [ATTACH PART\|PARTITION](#attach-partition) — Adds a part or partition from the `detached` directory to the table.
